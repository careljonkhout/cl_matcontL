function paper_testbruss_2d_BP2()
...
%%opt = contset(opt,'InitStepsize',   0.2); % n = 200,     N=10
%%opt = contset(opt,'MaxStepsize',    0.2); % n = 200,     N=10 
...
opt = contset(opt, 'Filename', 'paper_testbruss_2d_BP2');
...
load('Data\paper_testbruss_2d_BP1')
ID = 3; %user point

if strcmp(s(ID).label,'u1')
    data = s(ID).data;
    ap1  = 3;
    u = data.x(1:end-1);
