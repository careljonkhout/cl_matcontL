function paper_testbruss_2d_BP3()
...
opt = contset(opt,'InitStepsize',   5e-2); % n = 200,     N=10
opt = contset(opt,'MaxStepsize',    5e-2); % n = 200,     N=10 
...
opt = contset(opt, 'Filename', 'paper_testbruss_2d_BP3');
...
load('Data\paper_testbruss_2d_BP2')
ID = 2; %user point

if strcmp(s(ID).label,'u2')
    data = s(ID).data;
    ap1  = 4;
    u = data.x(1:end-1);
