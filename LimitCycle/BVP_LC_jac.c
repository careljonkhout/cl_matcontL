...
void mexFunction (int nlhs, mxArray *plhs[], int nrhs, const mxArray *prhs[]) {
  ...
  int nnz_estimate  = ncol * nphase * (ncol + 1) * nphase * ntst; // main blocks
      ...
      nnz_estimate += 2 * jacobian_height; // last two columns
      nnz_estimate += jacobian_width;      // bottom row
  ...
  if (nfreep==1)
    ...
    plhs[0] = mxCreateSparse(ncoords+1,ncoords+nfreep,nnz_estimate,mxREAL);
