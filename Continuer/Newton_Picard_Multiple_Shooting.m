...
function x = Newton_Picard_Correction(x0,v0)
  ...
  global cds
  [phases,period,parameters] = getComponents(x0);
  ...
  phi = zeros(m*cds.nphases,1);

  for j=1:m
    phases_i = phases((1:cds.nphases) + j * cds.nphases);
    cds.partial_trajectories(j) = ode15s(...
      @(t, y) cds.dydt_ode(t, y, parameters{:}), ...
      linspace(0, period/m, cds.nDiscretizationPoints), ...
      phases_i, integration_opt);
    phi(j) = deval(cds.partial_trajectory(j),period/m);
  end
  ...
  for i=1:m
    ...
    for j=1:basis_size
      [~,trajectory] = ode15s(dydt_partial_mon, [0 period], V(:,j), int_opt);
...
function V = compute_subspace(period, parameters)
  ...
  [eigenvectors, eigenvalues, no_convergence] = eigs( ...
    @(x) partial_monodromy_map(x, period, parameters), ...
  ...
  while p_eff < p && iteration < contopts.NewtonPicardMaxSubspaceIterations
    ...
    try 
      for i=p_eff+1:size(V_extended,2)
        % The function partial_monodromy_map cannot be used here, since it depends on
        % the global variable cds, and global variables are not copied so the
        % the workspace of the workers that parfor uses.
        [~,trajectory] = ode15s(...
...
% @phdthesis{lust-phd,
%	  author={Lust, Kurt},
%	  title={Numerical bifurcation analysis of periodic solutions of partial differential equations},
%	  school={K.U.Leuven},
%	  year={1997},
% }
% inputs:
