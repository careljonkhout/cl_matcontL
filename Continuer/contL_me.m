...
if isempty(firstpoint); print_diag(0,'contL: no convergence at x0.\n'); sout = []; return; end
firstpoint.h = cds.h;
...
%firstpoint.CISdata = feval(cds.curve_CIS_first_point, x0);
%if isempty(firstpoint.CISdata); print_diag(0,'contL: failed to intialize CIS algorithm.\n'); sout = []; return; end
...
%                     print_diag(0, 'contL: More than one singularity detected:\n')
    ...
    if NeedToLocate
        ...
        if ~isempty(singpoint)
            ...
            if ~failed
                ...
                failed = DefaultProcessor(singpoint, s);
            end
        else
        ...
        if any(userchanges)
            ...
            for ti=1:size(useridx,2)
                ...
                if isempty(userpoint)
                    ...
                    failed = DefaultProcessor(userpoint, s);
                end
            end
        end
    end
    ...
    tfUpdate = isfield(trialpoint, 'CISdata') && ~isempty(trialpoint.CISdata) && ...
...
global cds
failed = feval(cds.curve_defaultprocessor, varargin{:});
