function sol = bvp5c_mark(ode, bc, solinit, options)
...
[neqn,nparam,nregions,atol,rtol,Nmax,xyVectorized,printstats] = ...
    bvparguments_mark(solver_name,ode,bc,solinit,options);      % mark
   % bvparguments(solver_name,ode,bc,solinit,options);          % mark

% Modify equations to accommodate unknown parameters
[ode,bc,jac,bcjac,Joptions,dBCoptions] = ...        
    bvpfunctions_mark(solver_name,ode,bc,options,neqn,nparam,nregions); %mark
   % bvpfunctions(solver_name,ode,bc,options,neqn,nparam,nregions);     %mark

% Deal with a singular BVP.
[singularBVP,ode,jac,solinit,PBC] = ...
    bvpsingular_mark(solver_name,solinit,ode,jac,options,neqn,nparam,nregions); %mark
  % bvpsingular(solver_name,solinit,ode,jac,options,neqn,nparam,nregions);      %mark
...
%---------------------------------------------------------------------------
  ...
  % In a region, compute the ODE Jacobian at points in a mesh interval.          
    if isempty(jac)
      if isempty(Jpropagated)
        [Jn,Joptions.fac,~,nF] = ...
            odenumjac_mark(ode,{x(1),y(:,1),region{:}},f(:,1),Joptions); %mark
           %odenumjac(ode,{x(1),y(:,1),region{:}},f(:,1),Joptions);      %mark
        nODEeval = nODEeval + nF;
      else
        Jn = Jpropagated;          
      end   
      [Jnp1,Joptions.fac,~,nF] = ...
          odenumjac_mark(ode,{x(4),y(:,4),region{:}},f(:,4),Joptions);  %mark
        % odenumjac(ode,{x(4),y(:,4),region{:}},f(:,4),Joptions);       %mark
      nODEeval = nODEeval + nF;
      if norm(Jnp1 - Jn,1) <= 0.25*(norm(Jnp1,1) + norm(Jn,1)) 
        ...
        [Jnc1,Joptions.fac,~,nF1] = ...
            odenumjac_mark(ode,{x(2),y(:,2),region{:}},f(:,2),Joptions); %mark
          % odenumjac(ode,{x(2),y(:,2),region{:}},f(:,2),Joptions);      %mark
        [Jnc2,Joptions.fac,~,nF2] = ...
            odenumjac_mark(ode,{x(3),y(:,3),region{:}},f(:,3),Joptions); %mark
          % odenumjac(ode,{x(3),y(:,3),region{:}},f(:,3),Joptions);      %mark
  ...
  %BC_JAC  Compute the BC Jacobian.
    if isempty(bcjac)
      ...
      [dBCdya,dBCoptions.fac_dya,~,nF] = ...
            odenumjac_mark(@bcaux,{ya(:),yb(:)},bcVal,dBCoptions); % mark 
          % odenumjac(@bcaux,{ya(:),yb(:)},bcVal,dBCoptions);      % mark
      nBCeval = nBCeval + nF;
  ...
  % ymid is the solution at midpoints.      
    ...
    for i = 1 : numel(h)
      xres = x(i) + cres*h(i);
     %[yres,ypres] = ntrp4h(xres,x(i),y(:,i),x(i+1),y(:,i+1),...      %mark
      [yres,ypres] = ntrp4h_mark(xres,x(i),y(:,i),x(i+1),y(:,i+1),... %mark
                            ymid(:,i),yp(:,i),yp(:,i+1));
  ...
  % In a region, redistribute mesh points and approximate the solution.
    ...
    while i <= numel(errEst)
      ...
      if errEst(i) <= rtol
        ...
        yy = ntrp4h_mark(xx,xi,yi,xip1,yip1,ymid(:,i),yp(:,i),yp(:,i+1)); %mark
       %yy = ntrp4h(xx,xi,yi,xip1,yip1,ymid(:,i),yp(:,i),yp(:,i+1)); %mark
        ff =  odeFcn_region(xx,yy,region);        
        xx(  end+1) = xip1;
        yy(:,end+1) = yip1;
        ff(:,end+1) = fip1; 
