...
opt = contset(opt, 'Filename',     'testbratu_LP0');

%% Continuation 
N = 100; lambda = 0.45;
p = [N, lambda]; ap1 = 2;

%%[x0,v0]      = init_EP_EP_L(@bratu_1d, [], p, ap1);
[x0,v0]      = init_BVP_EP(@bratu_1d, [], p, ap1);
% init_BVP_EP
pause
contL(@equilibriumL,x0,v0,opt);

%% Plot results
x = loadPoint('Data\testbratu_LP0.dat');
load('Data\testbratu_LP0.mat')
N = s(1).data.P0(1);
plot(x(2*N+1, :), x(1, :));
hold on
for sii = s
