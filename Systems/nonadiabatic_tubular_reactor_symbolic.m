function out = nonadiabatic_tubular_reactor_symbolic
%
...
function du = fun_eval(~,x,D,P_em,P_eh,BETA,phi_0,GAMMA,B) % ignored parameter is t (time)
  ...
  u = reshape(x,2,N);
  for i=1:N
    du(:,i) = reaction_term(u(:,i)); %#ok<AGROW>
    % this methods is meant to be evaluated just a couple of times, therefore we
    % ignore the 'array will grow on every loop iteration warning'
  end
  P = [P_em; P_eh];
