...
void fusion_N_50_max_ord_1_jacobian_params(double* y, double* parameters, double* dydt);
...
void mexFunction(int nlhs, mxArray *plhs[], int nrhs, const mxArray *prhs[]) {
  ...
  if(nrhs != n_inputs) {
    mexErrMsgIdAndTxt("fusion_N_50_max_ord_1_dydt:nrhs","Five inputs required.");
  }
  if(nlhs != n_outputs) {
    mexErrMsgIdAndTxt("fusion_N_50_max_ord_1_dydt:nlhs","One output required.");
  }


  if ( !mxIsDouble(prhs[input_y]) ) {
    mexErrMsgIdAndTxt("system_fusion_N_50_max_ord_1:not_double",
                      "Error: Input vector y is not a double.");
  }

  if ( mxGetNumberOfElements(prhs[input_y]) != system_size ) {
    mexErrMsgIdAndTxt("system_fusion_N_50_max_ord_1:wrong_size",
                     "Input vector y must have 147 elements.");
  }

  for ( int i = 0; i < n_parameters; i++ ) {
    if ( !mxIsDouble(prhs[i + 2]) ) {
      mexErrMsgIdAndTxt("fusion_N_50_max_ord_1_dydt:not_double",
                        "Error: one of the parameters is not a double");
    } else if ( mxIsComplex(prhs[i + 2]) ) {
      mexErrMsgIdAndTxt("fusion_N_50_max_ord_1_dydt:not_complex",
                        "Error: one of the parameters not a real number");
    } else if ( mxGetNumberOfElements(prhs[i + 2]) != 1 ) {
      mexErrMsgIdAndTxt("fusion_N_50_max_ord_1_dydt:notScalar",
  ...
  fusion_N_50_max_ord_1_jacobian_params(y,parameters,outMatrix);
    
}

void fusion_N_50_max_ord_1_jacobian_params(double* y, double* parameters, double* jac) {
  jac[2] =  2.0E+1;
  jac[5] =  2.0E+1;
  jac[8] =  2.0E+1;
  jac[11] =  2.0E+1;
