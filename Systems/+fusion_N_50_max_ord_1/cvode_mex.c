...
static realtype    cycle_gap_tolerance;
static booleantype gap_tolerance_given;
...
void mexFunction(int n_output,       mxArray *mex_output[], 
  ...
  retval = CVodeSStolerances(cvode_mem, reltol, abs_tol);
...
static void check_double(const mxArray* array, char* arrayname) {
  if ( !mxIsDouble(array) ) {
    mexErrMsgIdAndTxt("cvode:not_double", 
            "Error: Input vector %s is not a vector of doubles.", arrayname);
  }
}
...
static void check_bool(const mxArray* array, char* arrayname) {
  if ( !mxIsLogicalScalar(array) ) {
    mexErrMsgIdAndTxt("cvode:not_double", 
            "Error: Input %s is not a boolean.", arrayname);
  }
}
...
//   if (from_index + data_length > size) {
//     grow_mxArray(array);
//   }
//   #if MX_HAS_INTERLEAVED_COMPLEX
//   mxDouble* dest = mxGetDoubles(array);
//   #else
//   mxDouble* dest = mxGetPr(     array);
//   #endif 
//   if (data_length > 1) {
//     memcpy(dest + from_index, data, data_length * sizeof(double));
//   } else {
//     dest[from_index] = *data;
//   }
// }
// 
// static void grow_mxArray(mxArray* array) {
//   #if MX_HAS_INTERLEAVED_COMPLEX
//   mxDouble* data = mxGetDoubles(array);
//   #else
//   mxDouble* data = mxGetPr(     array);
//   #endif 
//   int m     = mxGetM(array);
//   int n     = mxGetN(array);
//   int new_n = 2*n;
//   mexPrintf("new_n: %d", new_n);
//   data = mxRealloc(data, m * new_n * sizeof(double));
//   if (data == NULL) {
//     mexErrMsgIdAndTxt("cvode:out_of_memory", "Out of memory");
//   }
//   #if MX_HAS_INTERLEAVED_COMPLEX
//   mxSetDoubles(array, data);
//   #else
//   mxSetPr(     array, data);
//   #endif
//   mxSetN(array, new_n);
