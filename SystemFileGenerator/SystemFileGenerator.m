classdef SystemFileGenerator < matlab.mixin.CustomDisplay & handle
  ...
  methods
    ...
    function generate_c_files(s)
      ...
      sundails_path    = fullfile(mc_path, 'SystemFileGenerator', 'sundails');
      ...
      if any(strcmp({'cvode', 'cvode_sparse'}, s.output_type))
        ...
        sundails_sources = strrep(sundails_sources, sundails_path, '');

        add_fullfile = @(f) {sprintf('sundails_file(''%s'')', f)};
        sundails_sources = cellfun(add_fullfile, sundails_sources);
      ...
      build_script = [  sprintf([ ...
        ...
        '\n', ...
        '  function full_path = sundails_file(f)\n', ...
        '    full_path = fullfile(sundails_path, f);\n', ...
        '  end\n\n', ...
      ], s.name), build_script];
      ...
      recompile = sprintf('function compile()\n%s\nend', build_script);
