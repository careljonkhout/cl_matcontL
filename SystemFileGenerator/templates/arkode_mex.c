...
void mexFunction(int n_output,       mxArray *mex_output[], 
  ...
  arkstep = ARKStepCreate(NULL, dydt_cvode, t_values[0], solver_y);  
  
  ARKStepSetErrHandlerFn(arkstep, error_handler, NULL                 );
  ARKStepSetUserData    (arkstep, user_data                           );
  ARKStepSStolerances   (arkstep, rel_tol,       abs_tol              );
  ARKStepSetMaxNumSteps (arkstep, MAX_NUM_STEPS                       );
  ARKStepSetMaxHnilWarns(arkstep, 1                                   );
  #ifdef SUNDIALS_EXTENDED_PRECISION
  ARKStepSetMinStep     (arkstep, 1e-20q); // q mean quadruple precision
  #endif
  
  if (initial_step_size_given) {
    ARKStepSetInitStep(arkstep, initial_step_size);
  }
  ...
  ARKStepSetLinearSolver(arkstep, LS, A);
 
  /* Set the user-supplied Jacobian routine Jac */
  #if ANALYTIC_JACOBIAN
  ARKStepSetJacFn(arkstep, jacobian_dydt);
  #endif
  
  int rootfinding_direction = INCREASING;
  if (cycle_detection) {
    ARKStepRootInit        (arkstep, 1,                     return_to_plane);
    ARKStepSetRootDirection(arkstep, &rootfinding_direction                );
  }
  
  if (sensitivity) {
    mexErrMsgIdAndTxt("arkode:not_impl",
            "sensitivity not yet implemented for arkode");
...
//   }
  ...
  for(i = 1; i < n_points; i++) {
    ...
    int flag = ARKStepEvolve(arkstep, t_values[i], solver_y, &t, CV_NORMAL);
    check(flag, "CVode");
//     if (sensitivity) {
