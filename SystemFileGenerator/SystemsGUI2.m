classdef SystemsGUI2 < handle
    ...
    methods
        ...
        function app = SystemsGUI2
            ...
            app.createLabel(...
                [30 innerHeight - 2.5 * pitch labelwidth 14],'Parameters');
            app.createLabel(...
                [30 innerHeight - 3.5 * pitch labelwidth 14],'Time variable');
            app.createLabel(...
                [30 innerHeight - 4.75 * pitch labelwidth 14],...
                'Maximum order of symbolic derivatives');
            app.createLabel(...
                [30 innerHeight - 5.75* pitch labelwidth 14],'System equations');
            ...
            app.derivativesButtonGroup = uibuttongroup(app.Figure,...
                'Visible','off',...
                'Units','pixels',...
                'Position', [30+labelwidth+30, innerHeight - 5 * pitch,...
            ...
            app.equationsBox = uicontrol(app.Figure,...
                'Style','edit','Position',...
                [30 60 innerWidth - 2*30 innerHeight - 8 * pitch],...
                'Max',2,...
    ...
    function OKCallback(app,~,~)
      ...
      maxOrder = str2double(app.derivativesButtonGroup.SelectedObject.String);
      equations =  app.equationsBox.String;
      try
        app.progress = 0;
        app.myWaitbar = waitbar(app.progress, 'Please wait...');
        s = SystemFileGenerator(name, parameters, time, ...
        ...
        id = ex.identifier;
        if   strcmp(id, 'SystemFileGenerator:BadInput') ...
          || strcmp(id, 'SystemFileGenerator:EvalError') ...
          || strcmp(id, 'SystemFileGenerator:UndefinedFunctionInEval') ...
          || strcmp(id, 'SystemFileGenerator:expression_problem') ...
          || strcmp(id, 'SystemFileGenerator:invalid_lhs') ...
          || strcmp(id, 'SystemFileGenerator:too_much_equal_signs') 
