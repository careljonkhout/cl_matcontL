function [x0,v0] = init_LPC_LPC_L(odefile, x, s, ap, ntst, ncol,varargin)
  %
  % %
  ...
  x0(1:lds.ncoords+1) = x(1:lds.ncoords+1,1);
  ...
  for j=lds.tsts
    ...
    jac(range1,[range2 pars1]) = bordBVP_LPC_f(lds.func,xp,p,T,j); %#ok<SPRIX>
...
%-------------------------------------------------------------------------------
function init_lds(odefile,x,s,ap,~,~,varargin) 
  %unused arguments are ntst and ncol
  global lds
  lds=[];
  lds.odefile = odefile;
